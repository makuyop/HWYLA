...
public class HUDHandlerBlocks implements IComponentProvider {
    ...
    public void appendHead(List<Text> tooltip, IDataAccessor accessor, IPluginConfig config) {
        ...
        ((ITaggableList<Identifier, Text>) tooltip).setTag(OBJECT_NAME_TAG, new LiteralText(String.format(Waila.CONFIG.get().getFormatting().getBlockName(), accessor.getBlock().getName().asString())));
