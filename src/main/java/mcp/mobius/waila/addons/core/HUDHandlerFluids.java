...
public class HUDHandlerFluids implements IComponentProvider {
    ...
    public void appendHead(List<Text> tooltip, IDataAccessor accessor, IPluginConfig config) {
        ((ITaggableList<Identifier, Text>) tooltip).setTag(HUDHandlerBlocks.OBJECT_NAME_TAG, new LiteralText(String.format(Waila.CONFIG.get().getFormatting().getFluidName(), accessor.getBlock().getName().asString())));
